  ...
  while (true) {
    ...
    const items = await page.evaluate((categorias) => {
      return Array.from(document.querySelectorAll("div.item")).map((node) => {
        ...
        const datas = [];
        let lanceMinimo = "";
        const areaPattern = /(\d+([.,]\d+)?)\s*(m²|ha)/; // Padrão para capturar números com ou sem separador decimal seguidos por m² ou ha
        const areaMatch = title.match(areaPattern);
        const area = areaMatch ? areaMatch[0] : "Indisponível";
        ...
        return { title, auctionType, propertyType, datas, lanceMinimo, link, area };
      });
    }, categorias);
